...
// The reason that the function "nestedFunction" can access the variable "internal" is because the function is a closure. JavaScript is a
// lexically-scoped programming language so when console.log(internal); is called, the variable "interal" is looking for it's value inside of the "nestFunction"
// and whent it is not found, it will reference its outer enviornment. The variable is found at the next level and  will return its value which is defined inside of the function "myFunction".
...
const zooAnimals = [
  ...
  */
  ...
  function lowerCaseNames(arr){
    const arrLC = [];
     ...
     arrLC.push(`${item.animal_name.toLowerCase()}`)});
    return arrLC;
  }
  ...
  */
  ...
  function USApop(arr){
    const USpop = zooAnimals.reduce(function(accumulator, item){
